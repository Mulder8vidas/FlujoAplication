...
@Component({
  selector: 'app-consolidado-finito',
  templateUrl: './consolidado-finito.component.html',
  styleUrls: ['./consolidado-finito.component.css']
})
export class ConsolidadoFinitoComponent implements AfterViewInit{
  datasets:any[]=[];
  constructor(private simulacionService:SimulacionService) {
    ...
    this.ajustes=this.simulacionService.listAllSimulaciones[0].listaAjustes.filter(a=>a.majustes!="%").map(value => {
      return Math.log(Number(value.majustes)).toFixed(2);
    });
